...
/**
 * A host key that could not be accepted. The cases are distinct types rather than one type with a
 * nullable field: they carry the same fingerprint but call for opposite reactions from the user,
 * and a caller that forgets to branch should not compile.
 */
 ...
 * rebuilt, and the signature of an interception otherwise, which is why it is not the same type as
 * a first connection.
 */
 ...
 * Not the same as having no pin: the user verified this host once, and saying otherwise would hand
 * them the prompt they are likeliest to wave through.
 */
