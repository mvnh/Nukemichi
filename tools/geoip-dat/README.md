...
is only ever as fresh as the last time someone ran the regenerate step — there's no way to make
...
3. Commit `app/src/main/assets/geoip.dat` and the updated checksum **together**, in one commit —
   `preBuild`'s verify step fails the build for everyone else if they drift apart.
